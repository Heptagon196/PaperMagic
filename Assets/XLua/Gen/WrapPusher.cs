...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<NPC.CreatureFaction>(translator.PushNPCCreatureFaction, translator.Get, translator.UpdateNPCCreatureFaction);
				translator.RegisterPushAndGetAndUpdate<NPC.CreatureLevel>(translator.PushNPCCreatureLevel, translator.Get, translator.UpdateNPCCreatureLevel);
        ...
        int NPCCreatureFaction_TypeID = -1;
		int NPCCreatureFaction_EnumRef = -1;
        
        public void PushNPCCreatureFaction(RealStatePtr L, NPC.CreatureFaction val)
        {
            if (NPCCreatureFaction_TypeID == -1)
            {
			    bool is_first;
                NPCCreatureFaction_TypeID = getTypeId(L, typeof(NPC.CreatureFaction), out is_first);
				
				if (NPCCreatureFaction_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(NPC.CreatureFaction));
				    NPCCreatureFaction_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, NPCCreatureFaction_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, NPCCreatureFaction_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for NPC.CreatureFaction ,value="+val);
            }
			
			LuaAPI.lua_getref(L, NPCCreatureFaction_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out NPC.CreatureFaction val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != NPCCreatureFaction_TypeID)
				{
				    throw new Exception("invalid userdata for NPC.CreatureFaction");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for NPC.CreatureFaction");
                }
				val = (NPC.CreatureFaction)e;
                
            }
            else
            {
                val = (NPC.CreatureFaction)objectCasters.GetCaster(typeof(NPC.CreatureFaction))(L, index, null);
            }
        }
		
        public void UpdateNPCCreatureFaction(RealStatePtr L, int index, NPC.CreatureFaction val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for NPC.CreatureFaction ,value="+val);
                }
            }
        ...
        int NPCCreatureLevel_TypeID = -1;
		int NPCCreatureLevel_EnumRef = -1;
        
        public void PushNPCCreatureLevel(RealStatePtr L, NPC.CreatureLevel val)
        {
            if (NPCCreatureLevel_TypeID == -1)
            {
			    bool is_first;
                NPCCreatureLevel_TypeID = getTypeId(L, typeof(NPC.CreatureLevel), out is_first);
				
				if (NPCCreatureLevel_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(NPC.CreatureLevel));
				    NPCCreatureLevel_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, NPCCreatureLevel_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, NPCCreatureLevel_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for NPC.CreatureLevel ,value="+val);
            }
			
			LuaAPI.lua_getref(L, NPCCreatureLevel_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out NPC.CreatureLevel val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != NPCCreatureLevel_TypeID)
				{
				    throw new Exception("invalid userdata for NPC.CreatureLevel");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for NPC.CreatureLevel");
                }
				val = (NPC.CreatureLevel)e;
                
            }
            else
            {
                val = (NPC.CreatureLevel)objectCasters.GetCaster(typeof(NPC.CreatureLevel))(L, index, null);
            }
        }
		
        public void UpdateNPCCreatureLevel(RealStatePtr L, int index, NPC.CreatureLevel val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for NPC.CreatureLevel ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(NPC.CreatureFaction[]))
			{
			    NPC.CreatureFaction[] array = obj as NPC.CreatureFaction[];
				translator.PushNPCCreatureFaction(L, array[index]);
				return true;
			}
			else if (type == typeof(NPC.CreatureLevel[]))
			{
			    NPC.CreatureLevel[] array = obj as NPC.CreatureLevel[];
				translator.PushNPCCreatureLevel(L, array[index]);
				return true;
			}
